...
log() { printf '[source-resolve] %s\n' "$*"; }
fail() { printf '[source-resolve][error] %s\n' "$*" >&2; exit 1; }
...
check_url_reachable() {
  ...
  curl -fsSIL \
    ...
    --connect-timeout 15 \
    --max-time 60 \
    "${url}" >/dev/null || fail "${label} source URL is unreachable: ${url}"
}
...
if [[ "${CHECK_REMOTE}" == "1" ]]; then
  ...
  if [[ -n "${WCP_BIONIC_LAUNCHER_SOURCE_WCP_URL:-}" ]]; then
    check_url_reachable "${WCP_BIONIC_LAUNCHER_SOURCE_WCP_URL}" "Launcher"
  fi
  if [[ -n "${WCP_BIONIC_UNIX_SOURCE_WCP_URL:-}" ]]; then
    check_url_reachable "${WCP_BIONIC_UNIX_SOURCE_WCP_URL}" "Unix"
  fi
fi
