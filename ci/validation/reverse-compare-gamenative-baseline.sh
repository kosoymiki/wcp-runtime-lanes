...
resolve_tool_path() {
  ...
  for base in "${TOOLCHAIN_DIR:-}" "${LLVM_MINGW_DIR:-}"; do
    [[ -n "${base}" ]] || continue
    candidates+=(
      "${base}/bin/${tool}"
      "${base}/bin/x86_64-w64-mingw32-${tool}"
      "${base}/bin/aarch64-w64-mingw32-${tool}"
    )
  done
  candidates+=(
    "${tool}"
    "x86_64-w64-mingw32-${tool}"
    "aarch64-w64-mingw32-${tool}"
  )
...
main() {
  ...
  llvm_readobj_bin="$(resolve_tool_path llvm-readobj)" || fail "llvm-readobj not found (TOOLCHAIN_DIR=${TOOLCHAIN_DIR:-}, LLVM_MINGW_DIR=${LLVM_MINGW_DIR:-})"
  export LLVM_READOBJ="${llvm_readobj_bin}"
  log "Using llvm-readobj: ${LLVM_READOBJ}"
