...
: "${WLT_FINAL_STAGE_RUN_UNIFIED_PATCH_LANES:=1}"
: "${WLT_FINAL_STAGE_UNIFIED_PATCH_LANES_REQUIRED:=0}"
: "${WLT_FINAL_STAGE_RUN_UPSTREAM_PATCH_LANES:=0}"
...
[[ "${WLT_FINAL_STAGE_RUN_UNIFIED_PATCH_LANES}" =~ ^[01]$ ]] || fail "WLT_FINAL_STAGE_RUN_UNIFIED_PATCH_LANES must be 0 or 1"
[[ "${WLT_FINAL_STAGE_UNIFIED_PATCH_LANES_REQUIRED}" =~ ^[01]$ ]] || fail "WLT_FINAL_STAGE_UNIFIED_PATCH_LANES_REQUIRED must be 0 or 1"
[[ "${WLT_FINAL_STAGE_RUN_UPSTREAM_PATCH_LANES}" =~ ^[01]$ ]] || fail "WLT_FINAL_STAGE_RUN_UPSTREAM_PATCH_LANES must be 0 or 1"
...
unified_patch_lanes_rc=0
if [[ "${WLT_FINAL_STAGE_RUN_UNIFIED_PATCH_LANES}" == "1" ]]; then
  unified_patch_lanes_rc="$(run_capture unified-patch-lanes \
    env \
      WLT_UNIFIED_PATCH_OUT_DIR="${WLT_FINAL_STAGE_OUT_DIR}/unified-patch-lanes" \
      WLT_UNIFIED_PATCH_REQUIRED="${WLT_FINAL_STAGE_UNIFIED_PATCH_LANES_REQUIRED}" \
      bash "${ROOT_DIR}/ci/validation/check-unified-patch-lanes.sh")"
fi

upstream_patch_lanes_rc=0
if [[ "${WLT_FINAL_STAGE_RUN_UPSTREAM_PATCH_LANES}" == "1" ]]; then
  upstream_patch_lanes_rc="$(run_capture upstream-patch-lanes \
    env \
      WLT_UPSTREAM_PATCH_OUT_DIR="${WLT_FINAL_STAGE_OUT_DIR}/upstream-patch-lanes" \
      bash "${ROOT_DIR}/ci/validation/check-unified-patch-lanes-upstream.sh")"
fi
...

{
  ...
  printf 'run_unified_patch_lanes=%s\n' "${WLT_FINAL_STAGE_RUN_UNIFIED_PATCH_LANES}"
  printf 'unified_patch_lanes_required=%s\n' "${WLT_FINAL_STAGE_UNIFIED_PATCH_LANES_REQUIRED}"
  ...
  printf 'unified_patch_lanes_rc=%s\n' "${unified_patch_lanes_rc}"
  printf 'upstream_patch_lanes_rc=%s\n' "${upstream_patch_lanes_rc}"
...
if [[ "${WLT_FINAL_STAGE_FAIL_MODE}" == "strict" ]]; then
  if [[ "${reflective_rc}" != "0" || "${manifest_rc}" != "0" || "${contents_qa_rc}" != "0" || "${patch_base_rc}" != "0" || "${unified_patch_lanes_rc}" != "0" || "${upstream_patch_lanes_rc}" != "0" || "${wcp_parity_rc}" != "0" || "${intake_rc}" != "0" || "${high_cycle_rc}" != "0" || "${commit_scan_rc}" != "0" || "${release_prep_rc}" != "0" || "${snapshot_rc}" != "0" ]]; then
