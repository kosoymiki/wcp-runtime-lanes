...
ae_x11_contract_rc="$(run_capture ae-x11-contract \
  bash "${ROOT_DIR}/ci/winlator/check-ae-x11-contract.sh" "${WLT_PATCH_BASE_CONTRACT_PATCH_DIR}")"
...
if source_dir="$(pick_source_dir)"; then
  ...
  fi
  {
    ...
    printf 'ae_x11_contract_rc=%s\n' "${ae_x11_contract_rc}"
    printf 'patch_base_cycle_rc=0\n'
    printf 'patch_stack_rc=0\n'
    printf 'skipped=1\n'
  } > "${WLT_PATCH_BASE_CONTRACT_OUT_DIR}/summary.meta"
  if [[ "${ae_x11_contract_rc}" != "0" ]]; then
    fail "Ae/X11 patch identity contract failed (summary: ${WLT_PATCH_BASE_CONTRACT_OUT_DIR}/summary.meta)"
  fi
...
if [[ "${ae_x11_contract_rc}" != "0" || "${patch_base_cycle_rc}" != "0" || "${patch_stack_rc}" != "0" ]]; then
