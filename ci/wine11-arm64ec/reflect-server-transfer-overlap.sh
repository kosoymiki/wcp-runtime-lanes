...
SUPPORT_PREFIX_FILE="${ROOT_DIR}/ci/wine11-arm64ec/transfer-lanes/server-support/path-prefixes.txt"
...
: "${WCP_WINE11_SERVER_OVERLAP_OUT:=${ROOT_DIR}/docs/WINE11_ARM64EC_SERVER_TRANSFER_REPORT.md}"
: "${WCP_WINE11_SERVER_OVERLAP_WINDOW:=250}"
: "${WCP_WINE11_SERVER_OVERLAP_LIMIT:=40}"

log() { printf '[wine11-server-overlap] %s\n' "$*" >&2; }
fail() { printf '[wine11-server-overlap][error] %s\n' "$*" >&2; exit 1; }
tmp_dir=""
...
main() {
  ...
  local valve_ref valve_head lane_patch_count next_patch_count
  local -a pathspecs

  require_file "${PREFIX_FILE}"
  require_file "${SUPPORT_PREFIX_FILE}"
  ...
  tmp_dir="$(mktemp -d /tmp/wine11-server-overlap-XXXXXX)"
  ...
  git -C "${WCP_LOCAL_VALVE_WINE_EXP10_DIR}" log --format= --name-only --max-count "${WCP_WINE11_SERVER_OVERLAP_WINDOW}" "${valve_ref}" -- "${pathspecs[@]}" \
    | sed '/^$/d' \
    | sort \
    | uniq -c \
    | sort -nr \
    | head -n "${WCP_WINE11_SERVER_OVERLAP_LIMIT}" > "${hot_list}"
  ...
  next_patch_count="$((lane_patch_count + 1))"

  comm -12 "${valve_list}" "${gn_list}" > "${overlap_list}"

  mkdir -p "$(dirname "${WCP_WINE11_SERVER_OVERLAP_OUT}")"
  {
    printf '# Wine11 Server Transfer Report\n\n'
    printf 'This report narrows the Wine11 transfer work to the dedicated server-side lane.\n\n'
    printf '## Valve Source Window\n\n'
    printf '%s\n\n' "- Valve \`experimental_10.0\` head: \`${valve_head}\`"
    printf '%s\n\n' "- analyzed commit window: latest \`${WCP_WINE11_SERVER_OVERLAP_WINDOW}\` commits"

    printf '## Server Runtime Scope\n\n'
    printf '```text\n'
    cat "${PREFIX_FILE}"
    printf '```\n\n'

    printf '## Deferred Server Support Scope\n\n'
    printf '```text\n'
    cat "${SUPPORT_PREFIX_FILE}"
    printf '```\n\n'
    ...
    printf '## Current Server Runtime Files\n\n'
    printf '```text\n'
    if [[ -s "${lane_list}" ]]; then
      cat "${lane_list}"
    else
      printf '(no server-runtime patches yet)\n'
    fi
    printf '```\n\n'

    printf '## Interpretation\n\n'
    printf 'Server transfer work is separated because ownership across process/thread state and protocol layout is highly coupled.\n'
    printf 'Only isolated behavior fixes should land in `server-runtime`; protocol declarations stay deferred in `transfer-lanes/server-support` until a concrete slice needs them.\n'
    if [[ "${lane_patch_count}" -gt 0 ]]; then
      printf 'The lane is currently intentionally frozen at `%s` runtime patch(es): the latest full stacked re-check did not prove a safe additional protocol-free `server%s` hunk inside `server/process.c` or `server/thread.c`.\n' "${lane_patch_count}" "${next_patch_count}"
    fi
  } > "${WCP_WINE11_SERVER_OVERLAP_OUT}"

  log "Report written: ${WCP_WINE11_SERVER_OVERLAP_OUT}"
}
