...
: "${WCP_WINE11_WOW64_STRUCT_SUPPORT_OVERLAP_OUT:=${ROOT_DIR}/docs/WINE11_ARM64EC_WOW64_STRUCT_SUPPORT_REPORT.md}"
: "${WCP_WINE11_WOW64_STRUCT_SUPPORT_OVERLAP_WINDOW:=250}"
: "${WCP_WINE11_WOW64_STRUCT_SUPPORT_OVERLAP_LIMIT:=40}"

log() { printf '[wine11-wow64-struct-support-overlap] %s\n' "$*" >&2; }
fail() { printf '[wine11-wow64-struct-support-overlap][error] %s\n' "$*" >&2; exit 1; }
tmp_dir=""
...
main() {
  ...
  tmp_dir="$(mktemp -d /tmp/wine11-wow64-struct-support-overlap-XXXXXX)"
  ...
  git -C "${WCP_LOCAL_VALVE_WINE_EXP10_DIR}" log --format= --name-only --max-count "${WCP_WINE11_WOW64_STRUCT_SUPPORT_OVERLAP_WINDOW}" "${valve_ref}" -- "${pathspecs[@]}" \
    | sed '/^$/d' \
    | sort \
    | uniq -c \
    | sort -nr \
    | head -n "${WCP_WINE11_WOW64_STRUCT_SUPPORT_OVERLAP_LIMIT}" > "${hot_list}"
  ...
  mkdir -p "$(dirname "${WCP_WINE11_WOW64_STRUCT_SUPPORT_OVERLAP_OUT}")"
  {
    printf '# Wine11 Wow64 Struct Support Report\n\n'
    printf 'This report tracks the deferred `wow64-struct-support` lane for narrow 32-bit bridge structures.\n\n'
    printf '## Valve Source Window\n\n'
    printf '%s\n\n' "- Valve \`experimental_10.0\` head: \`${valve_head}\`"
    printf '%s\n\n' "- analyzed commit window: latest \`${WCP_WINE11_WOW64_STRUCT_SUPPORT_OVERLAP_WINDOW}\` commits"

    printf '## Wow64 Struct Support Scope\n\n'
    printf '```text\n'
    cat "${PREFIX_FILE}"
    printf '```\n\n'
    ...
    printf '## Current Wow64 Struct Support Files\n\n'
    printf '```text\n'
    if [[ -s "${lane_list}" ]]; then
      cat "${lane_list}"
    else
      printf '(no wow64-struct-support patches yet)\n'
    fi
    printf '```\n\n'

    printf '## Interpretation\n\n'
    printf 'Keep `dlls/wow64/struct32.h` isolated until a concrete wow64/core bridge slice truly needs 32-bit struct support.\n'
  } > "${WCP_WINE11_WOW64_STRUCT_SUPPORT_OVERLAP_OUT}"

  log "Report written: ${WCP_WINE11_WOW64_STRUCT_SUPPORT_OVERLAP_OUT}"
}
