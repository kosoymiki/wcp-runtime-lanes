...
LANE_DIR="${ROOT_DIR}/ci/wine11-arm64ec/transfer-lanes/signal-runtime"
PREFIX_FILE="${LANE_DIR}/path-prefixes.txt"
PATCH_DIR="${LANE_DIR}/patches"
SUPPORT_PREFIX_FILE="${ROOT_DIR}/ci/wine11-arm64ec/transfer-lanes/signal-support/path-prefixes.txt"
...
: "${WCP_WINE11_SIGNAL_OVERLAP_OUT:=${ROOT_DIR}/docs/WINE11_ARM64EC_SIGNAL_TRANSFER_REPORT.md}"
: "${WCP_WINE11_SIGNAL_OVERLAP_WINDOW:=250}"
: "${WCP_WINE11_SIGNAL_OVERLAP_LIMIT:=40}"

log() { printf '[wine11-signal-overlap] %s\n' "$*" >&2; }
fail() { printf '[wine11-signal-overlap][error] %s\n' "$*" >&2; exit 1; }
tmp_dir=""
...
main() {
  ...
  tmp_dir="$(mktemp -d /tmp/wine11-signal-overlap-XXXXXX)"
  ...
  git -C "${WCP_LOCAL_VALVE_WINE_EXP10_DIR}" log --format= --name-only --max-count "${WCP_WINE11_SIGNAL_OVERLAP_WINDOW}" "${valve_ref}" -- "${pathspecs[@]}" \
    | sed '/^$/d' \
    | sort \
    | uniq -c \
    | sort -nr \
    | head -n "${WCP_WINE11_SIGNAL_OVERLAP_LIMIT}" > "${hot_list}"
  ...
  mkdir -p "$(dirname "${WCP_WINE11_SIGNAL_OVERLAP_OUT}")"
  {
    printf '# Wine11 Signal Transfer Report\n\n'
    printf 'This report narrows the Wine11 transfer work to the dedicated `ntdll` signal lane.\n\n'
    printf '## Valve Source Window\n\n'
    printf '%s\n\n' "- Valve \`experimental_10.0\` head: \`${valve_head}\`"
    printf '%s\n\n' "- analyzed commit window: latest \`${WCP_WINE11_SIGNAL_OVERLAP_WINDOW}\` commits"

    printf '## Signal Runtime Scope\n\n'
    printf '```text\n'
    cat "${PREFIX_FILE}"
    printf '```\n\n'

    printf '## Deferred Signal Support Scope\n\n'
    printf '```text\n'
    ...
    printf '## Current Signal Runtime Files\n\n'
    printf '```text\n'
    if [[ -s "${lane_list}" ]]; then
      cat "${lane_list}"
    else
      printf '(no signal-runtime patches yet)\n'
    fi
    printf '```\n\n'

    printf '## Interpretation\n\n'
    printf 'The signal lane is kept separate because unwind, exception, and context ownership is highly regression-prone.\n'
    printf 'Only isolated signal-handler fixes should land here; declaration/export support stays deferred in `transfer-lanes/signal-support` until a concrete slice needs it.\n'
  } > "${WCP_WINE11_SIGNAL_OVERLAP_OUT}"

  log "Report written: ${WCP_WINE11_SIGNAL_OVERLAP_OUT}"
}
