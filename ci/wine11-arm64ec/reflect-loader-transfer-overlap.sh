...
LANE_DIR="${ROOT_DIR}/ci/wine11-arm64ec/transfer-lanes/loader-runtime"
...
SUPPORT_PREFIX_FILE="${ROOT_DIR}/ci/wine11-arm64ec/transfer-lanes/loader-support/path-prefixes.txt"
...
: "${WCP_WINE11_LOADER_OVERLAP_OUT:=${ROOT_DIR}/docs/WINE11_ARM64EC_LOADER_TRANSFER_REPORT.md}"
: "${WCP_WINE11_LOADER_OVERLAP_WINDOW:=200}"
: "${WCP_WINE11_LOADER_OVERLAP_LIMIT:=20}"

log() { printf '[wine11-loader-overlap] %s\n' "$*" >&2; }
fail() { printf '[wine11-loader-overlap][error] %s\n' "$*" >&2; exit 1; }
tmp_dir=""
...
main() {
  local valve_list gn_list lane_list core_list hot_list overlap_list
  ...
  git -C "${WCP_LOCAL_VALVE_WINE_EXP10_DIR}" log --format= --name-only --max-count "${WCP_WINE11_LOADER_OVERLAP_WINDOW}" "${valve_ref}" -- "${pathspecs[@]}" \
    | sed '/^$/d' \
    | sort \
    | uniq -c \
    | sort -nr \
    | head -n "${WCP_WINE11_LOADER_OVERLAP_LIMIT}" > "${hot_list}"
  ...
  find "${CORE_PATCH_DIR}" -maxdepth 1 -type f -name '*.patch' | sort | while IFS= read -r patch_file; do
    extract_patch_paths "${patch_file}"
  done | sort -u > "${core_list}"
  ...
  mkdir -p "$(dirname "${WCP_WINE11_LOADER_OVERLAP_OUT}")"
  {
    printf '# Wine11 Loader Transfer Report\n\n'
    printf 'This report narrows the Wine11 transfer work to the dedicated loader runtime lane.\n\n'
    printf '## Valve Source Window\n\n'
    printf '%s\n\n' "- Valve \`experimental_10.0\` head: \`${valve_head}\`"
    printf '%s\n\n' "- analyzed commit window: latest \`${WCP_WINE11_LOADER_OVERLAP_WINDOW}\` commits"

    printf '## Loader Runtime Scope\n\n'
    printf '```text\n'
    cat "${PREFIX_FILE}"
    printf '```\n\n'

    printf '## Deferred Loader Support Scope\n\n'
    printf '```text\n'
    ...
    printf '## Current Core Runtime Context\n\n'
    printf '```text\n'
    if [[ -s "${core_list}" ]]; then
      cat "${core_list}"
    else
      printf '(no core-runtime patches touch this scope)\n'
    fi
    printf '```\n\n'

    printf '## Current Loader Runtime Files\n\n'
    printf '```text\n'
    if [[ -s "${lane_list}" ]]; then
      cat "${lane_list}"
    else
      printf '(no loader-runtime patches yet)\n'
    fi
    printf '```\n\n'

    printf '## Interpretation\n\n'
    printf 'The loader runtime lane stays pinned to one file so that loader-only behavior can advance without dragging `ntdll.spec` or `ntdll_misc.h` prematurely.\n'
    printf 'When a loader slice needs a new export or declaration, it must move through `transfer-lanes/loader-support` as a separate preparatory step.\n'
  } > "${WCP_WINE11_LOADER_OVERLAP_OUT}"

  log "Report written: ${WCP_WINE11_LOADER_OVERLAP_OUT}"
}
