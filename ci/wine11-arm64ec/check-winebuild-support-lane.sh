...
log() { printf '[wine11-winebuild-support] %s\n' "$*"; }
fail() { printf '[wine11-winebuild-support][error] %s\n' "$*" >&2; exit 1; }
tmp_dir=""
...
main() {
  ...
  tmp_dir="$(mktemp -d /tmp/wine11-winebuild-support-XXXXXX)"
  trap 'test -n "${tmp_dir-}" && rm -rf "${tmp_dir}"' EXIT
  clone_dir="${tmp_dir}/src"
  out_dir="${tmp_dir}/out"
  report_file="${out_dir}/gamenative-patchset-wine11-winebuild-support.tsv"
  ...
  if [[ "${#patch_files[@]}" -eq 0 ]]; then
    log "No winebuild-support patches yet; libs-wine-support + wow64-struct-support + wow64-support + server-runtime + signal-runtime + loader-runtime + core-runtime + GN baseline preflight passed"
    return 0
  fi
  ...
  for patch in "${patch_files[@]}"; do
    ...
    while IFS= read -r changed_file; do
      ...
      path_in_scope "${changed_file}" || fail "Out-of-scope path touched by winebuild-support lane: ${changed_file}"
  ...
  log "Winebuild support lane passed: ${patch_count} custom patch(es) on top of libs-wine-support + wow64-struct-support + wow64-support + server-runtime + signal-runtime + loader-runtime + core-runtime + GN baseline"
}
