...
log() { printf '[wine11-loader-support] %s\n' "$*"; }
fail() { printf '[wine11-loader-support][error] %s\n' "$*" >&2; exit 1; }
tmp_dir=""
...
main() {
  ...
  tmp_dir="$(mktemp -d /tmp/wine11-loader-support-XXXXXX)"
  trap 'test -n "${tmp_dir-}" && rm -rf "${tmp_dir}"' EXIT
  clone_dir="${tmp_dir}/src"
  out_dir="${tmp_dir}/out"
  report_file="${out_dir}/gamenative-patchset-wine11-loader-support.tsv"
  ...
  if [[ "${#patch_files[@]}" -eq 0 ]]; then
    log "No loader-support patches yet; loader-runtime + core-runtime + GN baseline preflight passed"
    return 0
  fi
  ...
  for patch in "${patch_files[@]}"; do
    ...
    while IFS= read -r changed_file; do
      ...
      path_in_scope "${changed_file}" || fail "Out-of-scope path touched by loader-support lane: ${changed_file}"
  ...
  log "Loader support lane passed: ${patch_count} custom patch(es) on top of loader-runtime + core-runtime + GN baseline"
}
