...
LANE_DIR="${ROOT_DIR}/ci/wine11-arm64ec/transfer-lanes/nls-po-programs-server-tools-wave1-runtime"
...
: "${WCP_WINE11_NPSST_WAVE1_RUNTIME_OVERLAP_OUT:=${ROOT_DIR}/docs/WINE11_ARM64EC_NLS_PO_PROGRAMS_SERVER_TOOLS_WAVE1_RUNTIME_REPORT.md}"
: "${WCP_WINE11_NPSST_WAVE1_RUNTIME_OVERLAP_WINDOW:=300}"
: "${WCP_WINE11_NPSST_WAVE1_RUNTIME_OVERLAP_LIMIT:=80}"

log() { printf '[wine11-npsst-wave1-runtime-overlap] %s\n' "$*" >&2; }
fail() { printf '[wine11-npsst-wave1-runtime-overlap][error] %s\n' "$*" >&2; exit 1; }
tmp_dir=""
...
main() {
  ...
  tmp_dir="$(mktemp -d /tmp/wine11-npsst-wave1-runtime-overlap-XXXXXX)"
  ...
  git -C "${WCP_LOCAL_VALVE_WINE_EXP10_DIR}" log --format= --name-only --max-count "${WCP_WINE11_NPSST_WAVE1_RUNTIME_OVERLAP_WINDOW}" "${valve_ref}" -- "${pathspecs[@]}" \
    | sed '/^$/d' | sort | uniq -c | sort -nr | head -n "${WCP_WINE11_NPSST_WAVE1_RUNTIME_OVERLAP_LIMIT}" > "${hot_list}"
  ...
  mkdir -p "$(dirname "${WCP_WINE11_NPSST_WAVE1_RUNTIME_OVERLAP_OUT}")"
  {
    printf '# Wine11 Nls Po Programs Server Tools Wave1 Runtime Report\n\n'
    printf 'This report tracks the opened `nls + po + programs + server + tools` transfer lane after `include-libs-wave1-runtime`.\n\n'
    printf '## Valve Source Window\n\n'
    printf '%s\n\n' "- Valve \`experimental_10.0\` head: \`${valve_head}\`"
    printf '%s\n\n' "- analyzed commit window: latest \`${WCP_WINE11_NPSST_WAVE1_RUNTIME_OVERLAP_WINDOW}\` commits"

    printf '## Nls Po Programs Server Tools Wave1 Runtime Scope\n\n'
    printf '```text\n'
    cat "${PREFIX_FILE}"
    printf '```\n\n'
    ...
    printf '## Current Nls Po Programs Server Tools Wave1 Runtime Files\n\n'
    printf '```text\n'
    if [[ -s "${lane_list}" ]]; then
      cat "${lane_list}"
    else
      printf '(no nls-po-programs-server-tools-wave1-runtime patches yet)\n'
    fi
    printf '```\n\n'

    printf '## Interpretation\n\n'
    printf 'Keep `nls + po + programs + server + tools` changes isolated in this lane and preserve prior ownership boundaries.\n'
    printf 'The lane is currently tracked with `%s` landed patch slice(s).\n' "${lane_patch_count}"
  } > "${WCP_WINE11_NPSST_WAVE1_RUNTIME_OVERLAP_OUT}"

  log "Report written: ${WCP_WINE11_NPSST_WAVE1_RUNTIME_OVERLAP_OUT}"
}
