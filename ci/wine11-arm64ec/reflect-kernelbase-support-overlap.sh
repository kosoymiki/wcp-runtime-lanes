...
LANE_DIR="${ROOT_DIR}/ci/wine11-arm64ec/transfer-lanes/kernelbase-support"
...
: "${WCP_WINE11_KERNELBASE_SUPPORT_OVERLAP_OUT:=${ROOT_DIR}/docs/WINE11_ARM64EC_KERNELBASE_SUPPORT_REPORT.md}"
: "${WCP_WINE11_KERNELBASE_SUPPORT_OVERLAP_WINDOW:=250}"
: "${WCP_WINE11_KERNELBASE_SUPPORT_OVERLAP_LIMIT:=20}"

log() { printf '[wine11-kernelbase-support-overlap] %s\n' "$*" >&2; }
fail() { printf '[wine11-kernelbase-support-overlap][error] %s\n' "$*" >&2; exit 1; }
tmp_dir=""
...
main() {
  ...
  tmp_dir="$(mktemp -d /tmp/wine11-kernelbase-support-overlap-XXXXXX)"
  ...
  git -C "${WCP_LOCAL_VALVE_WINE_EXP10_DIR}" log --format= --name-only --max-count "${WCP_WINE11_KERNELBASE_SUPPORT_OVERLAP_WINDOW}" "${valve_ref}" -- "${pathspecs[@]}" \
    | sed '/^$/d' | sort | uniq -c | sort -nr | head -n "${WCP_WINE11_KERNELBASE_SUPPORT_OVERLAP_LIMIT}" > "${hot_list}"
  ...
  mkdir -p "$(dirname "${WCP_WINE11_KERNELBASE_SUPPORT_OVERLAP_OUT}")"
  {
    printf '# Wine11 Kernelbase Support Report\n\n'
    printf 'This report tracks the opened `kernelbase` support lane.\n\n'
    printf '## Valve Source Window\n\n'
    printf '%s\n\n' "- Valve \`experimental_10.0\` head: \`${valve_head}\`"
    printf '%s\n\n' "- analyzed commit window: latest \`${WCP_WINE11_KERNELBASE_SUPPORT_OVERLAP_WINDOW}\` commits"
    printf '## Kernelbase Support Scope\n\n```text\n'
    cat "${PREFIX_FILE}"
    ...
    printf '```\n\n## Current Kernelbase Support Files\n\n```text\n'
    if [[ -s "${lane_list}" ]]; then cat "${lane_list}"; else printf '(no kernelbase-support patches yet)\n'; fi
    printf '```\n\n## Interpretation\n\n'
    printf 'Keep `kernelbase-support` empty until a concrete `kernelbase-runtime` slice proves a spec/export dependency; opening the lane alone is not a reason to add support patches.\n'
  } > "${WCP_WINE11_KERNELBASE_SUPPORT_OVERLAP_OUT}"

  log "Report written: ${WCP_WINE11_KERNELBASE_SUPPORT_OVERLAP_OUT}"
}
