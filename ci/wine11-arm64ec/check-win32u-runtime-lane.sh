...
log() { printf '[wine11-win32u-runtime] %s\n' "$*"; }
fail() { printf '[wine11-win32u-runtime][error] %s\n' "$*" >&2; exit 1; }
tmp_dir=""
...
main() {
  ...
  tmp_dir="$(mktemp -d /tmp/wine11-win32u-runtime-XXXXXX)"
  trap 'test -n "${tmp_dir-}" && rm -rf "${tmp_dir}"' EXIT
  clone_dir="${tmp_dir}/src"
  out_dir="${tmp_dir}/out"
  report_file="${out_dir}/gamenative-patchset-wine11-win32u-runtime.tsv"
  ...
  if [[ "${#patch_files[@]}" -eq 0 ]]; then
    log "No win32u-runtime patches yet; kernelbase-support + kernelbase-runtime + winebuild-support + libs-wine-support + wow64-struct-support + wow64-support + server-runtime + signal-runtime + loader-runtime + core-runtime + GN baseline preflight passed"
    return 0
  fi
  ...
  for patch in "${patch_files[@]}"; do
    ...
    while IFS= read -r changed_file; do
      ...
      path_in_scope "${changed_file}" || fail "Out-of-scope path touched by win32u-runtime lane: ${changed_file}"
  ...
  log "Win32u runtime lane passed: ${patch_count} custom patch(es) on top of kernelbase-support + kernelbase-runtime + winebuild-support + libs-wine-support + wow64-struct-support + wow64-support + server-runtime + signal-runtime + loader-runtime + core-runtime + GN baseline"
}
