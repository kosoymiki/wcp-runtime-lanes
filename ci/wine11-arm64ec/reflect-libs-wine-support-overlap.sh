...
LANE_DIR="${ROOT_DIR}/ci/wine11-arm64ec/transfer-lanes/libs-wine-support"
...
: "${WCP_WINE11_LIBS_WINE_SUPPORT_OVERLAP_OUT:=${ROOT_DIR}/docs/WINE11_ARM64EC_LIBS_WINE_SUPPORT_REPORT.md}"
: "${WCP_WINE11_LIBS_WINE_SUPPORT_OVERLAP_WINDOW:=250}"
: "${WCP_WINE11_LIBS_WINE_SUPPORT_OVERLAP_LIMIT:=40}"

log() { printf '[wine11-libs-wine-support-overlap] %s\n' "$*" >&2; }
fail() { printf '[wine11-libs-wine-support-overlap][error] %s\n' "$*" >&2; exit 1; }
tmp_dir=""
...
main() {
  ...
  tmp_dir="$(mktemp -d /tmp/wine11-libs-wine-support-overlap-XXXXXX)"
  ...
  git -C "${WCP_LOCAL_VALVE_WINE_EXP10_DIR}" log --format= --name-only --max-count "${WCP_WINE11_LIBS_WINE_SUPPORT_OVERLAP_WINDOW}" "${valve_ref}" -- "${pathspecs[@]}" \
    | sed '/^$/d' \
    | sort \
    | uniq -c \
    | sort -nr \
    | head -n "${WCP_WINE11_LIBS_WINE_SUPPORT_OVERLAP_LIMIT}" > "${hot_list}"
  ...
  mkdir -p "$(dirname "${WCP_WINE11_LIBS_WINE_SUPPORT_OVERLAP_OUT}")"
  {
    printf '# Wine11 Libs Wine Support Report\n\n'
    printf 'This report tracks the deferred `libs/wine` support lane.\n\n'
    printf '## Valve Source Window\n\n'
    printf '%s\n\n' "- Valve \`experimental_10.0\` head: \`${valve_head}\`"
    printf '%s\n\n' "- analyzed commit window: latest \`${WCP_WINE11_LIBS_WINE_SUPPORT_OVERLAP_WINDOW}\` commits"

    printf '## Libs Wine Support Scope\n\n'
    printf '```text\n'
    cat "${PREFIX_FILE}"
    printf '```\n\n'
    ...
    printf '## Current Libs Wine Support Files\n\n'
    printf '```text\n'
    if [[ -s "${lane_list}" ]]; then
      cat "${lane_list}"
    else
      printf '(no libs-wine-support patches yet)\n'
    fi
    printf '```\n\n'

    printf '## Interpretation\n\n'
    printf 'Keep `libs/wine` changes isolated until an upstream transfer truly requires helper-library side changes.\n'
  } > "${WCP_WINE11_LIBS_WINE_SUPPORT_OVERLAP_OUT}"

  log "Report written: ${WCP_WINE11_LIBS_WINE_SUPPORT_OVERLAP_OUT}"
}
