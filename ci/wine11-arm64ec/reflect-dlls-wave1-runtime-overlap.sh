...
LANE_DIR="${ROOT_DIR}/ci/wine11-arm64ec/transfer-lanes/dlls-wave1-runtime"
...
: "${WCP_WINE11_DLLS_WAVE1_RUNTIME_OVERLAP_OUT:=${ROOT_DIR}/docs/WINE11_ARM64EC_DLLS_WAVE1_RUNTIME_REPORT.md}"
: "${WCP_WINE11_DLLS_WAVE1_RUNTIME_OVERLAP_WINDOW:=250}"
: "${WCP_WINE11_DLLS_WAVE1_RUNTIME_OVERLAP_LIMIT:=40}"

log() { printf '[wine11-dlls-wave1-runtime-overlap] %s\n' "$*" >&2; }
fail() { printf '[wine11-dlls-wave1-runtime-overlap][error] %s\n' "$*" >&2; exit 1; }
tmp_dir=""
...
main() {
  ...
  tmp_dir="$(mktemp -d /tmp/wine11-dlls-wave1-runtime-overlap-XXXXXX)"
  ...
  git -C "${WCP_LOCAL_VALVE_WINE_EXP10_DIR}" log --format= --name-only --max-count "${WCP_WINE11_DLLS_WAVE1_RUNTIME_OVERLAP_WINDOW}" "${valve_ref}" -- "${pathspecs[@]}" \
    | sed '/^$/d' | sort | uniq -c | sort -nr | head -n "${WCP_WINE11_DLLS_WAVE1_RUNTIME_OVERLAP_LIMIT}" > "${hot_list}"
  ...
  mkdir -p "$(dirname "${WCP_WINE11_DLLS_WAVE1_RUNTIME_OVERLAP_OUT}")"
  {
    printf '# Wine11 DLLs Wave1 Runtime Report\n\n'
    printf 'This report tracks the opened donor-bundle runtime lane after `kernel32`.\n\n'
    printf '## Valve Source Window\n\n'
    printf '%s\n\n' "- Valve \`experimental_10.0\` head: \`${valve_head}\`"
    printf '%s\n\n' "- analyzed commit window: latest \`${WCP_WINE11_DLLS_WAVE1_RUNTIME_OVERLAP_WINDOW}\` commits"
    printf '## DLLs Wave1 Runtime Scope\n\n```text\n'
    cat "${PREFIX_FILE}"
    ...
    printf '```\n\n## Current DLLs Wave1 Runtime Files\n\n```text\n'
    if [[ -s "${lane_list}" ]]; then cat "${lane_list}"; else printf '(no dlls-wave1-runtime patches yet)\n'; fi
    printf '```\n\n## Interpretation\n\n'
    printf 'Keep `dlls-wave1-runtime` strictly inside the requested prefix bundle and land only bounded\n'
    printf 'file-local runtime slices. The lane is currently proven with `%s` landed runtime slices.\n' "${lane_patch_count}"
  } > "${WCP_WINE11_DLLS_WAVE1_RUNTIME_OVERLAP_OUT}"

  log "Report written: ${WCP_WINE11_DLLS_WAVE1_RUNTIME_OVERLAP_OUT}"
}
