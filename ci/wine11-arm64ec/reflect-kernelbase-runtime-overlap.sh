...
LANE_DIR="${ROOT_DIR}/ci/wine11-arm64ec/transfer-lanes/kernelbase-runtime"
...
: "${WCP_WINE11_KERNELBASE_RUNTIME_OVERLAP_OUT:=${ROOT_DIR}/docs/WINE11_ARM64EC_KERNELBASE_RUNTIME_REPORT.md}"
: "${WCP_WINE11_KERNELBASE_RUNTIME_OVERLAP_WINDOW:=250}"
: "${WCP_WINE11_KERNELBASE_RUNTIME_OVERLAP_LIMIT:=30}"

log() { printf '[wine11-kernelbase-runtime-overlap] %s\n' "$*" >&2; }
fail() { printf '[wine11-kernelbase-runtime-overlap][error] %s\n' "$*" >&2; exit 1; }
tmp_dir=""
...
main() {
  ...
  tmp_dir="$(mktemp -d /tmp/wine11-kernelbase-runtime-overlap-XXXXXX)"
  ...
  git -C "${WCP_LOCAL_VALVE_WINE_EXP10_DIR}" log --format= --name-only --max-count "${WCP_WINE11_KERNELBASE_RUNTIME_OVERLAP_WINDOW}" "${valve_ref}" -- "${pathspecs[@]}" \
    | sed '/^$/d' | sort | uniq -c | sort -nr | head -n "${WCP_WINE11_KERNELBASE_RUNTIME_OVERLAP_LIMIT}" > "${hot_list}"
  ...
  mkdir -p "$(dirname "${WCP_WINE11_KERNELBASE_RUNTIME_OVERLAP_OUT}")"
  {
    printf '# Wine11 Kernelbase Runtime Report\n\n'
    printf 'This report tracks the opened first non-core runtime lane after the frozen Wine11 core block.\n\n'
    printf '## Valve Source Window\n\n'
    printf '%s\n\n' "- Valve \`experimental_10.0\` head: \`${valve_head}\`"
    printf '%s\n\n' "- analyzed commit window: latest \`${WCP_WINE11_KERNELBASE_RUNTIME_OVERLAP_WINDOW}\` commits"
    printf '## Kernelbase Runtime Scope\n\n```text\n'
    cat "${PREFIX_FILE}"
    ...
    printf '```\n\n## Current Kernelbase Runtime Files\n\n```text\n'
    if [[ -s "${lane_list}" ]]; then cat "${lane_list}"; else printf '(no kernelbase-runtime patches yet)\n'; fi
    printf '```\n\n## Interpretation\n\n'
    printf 'Keep `kernelbase-runtime` file-bounded, downstream of the already-frozen core block, and explicitly\n'
    printf 'opened as the first deferred release-library lane. The lane is currently proven with `%s` landed\n' "${lane_patch_count}"
    printf 'file-local runtime slices and no required `kernelbase-support` dependency yet.\n'
  } > "${WCP_WINE11_KERNELBASE_RUNTIME_OVERLAP_OUT}"

  log "Report written: ${WCP_WINE11_KERNELBASE_RUNTIME_OVERLAP_OUT}"
}
