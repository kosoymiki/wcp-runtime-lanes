...
log() { printf '[wine11-npsst-wave1-runtime] %s\n' "$*"; }
fail() { printf '[wine11-npsst-wave1-runtime][error] %s\n' "$*" >&2; exit 1; }
tmp_dir=""
...
main() {
  ...
  tmp_dir="$(mktemp -d /tmp/wine11-npsst-wave1-runtime-XXXXXX)"
  trap 'test -n "${tmp_dir-}" && rm -rf "${tmp_dir}"' EXIT
  clone_dir="${tmp_dir}/src"
  out_dir="${tmp_dir}/out"
  report_file="${out_dir}/gamenative-patchset-wine11-npsst-wave1-runtime.tsv"
  ...
  if [[ "${#patch_files[@]}" -eq 0 ]]; then
    log "No nls-po-programs-server-tools-wave1-runtime patches yet; include-libs-wave1-runtime + dlls-wave1-runtime + kernel32-runtime + win32u-runtime + kernelbase-support + kernelbase-runtime + winebuild-support + libs-wine-support + wow64-struct-support + wow64-support + server-runtime + signal-runtime + loader-runtime + core-runtime + GN baseline preflight passed"
    return 0
  fi
  ...
  for patch in "${patch_files[@]}"; do
    ...
    while IFS= read -r changed_file; do
      ...
      path_in_scope "${changed_file}" || fail "Out-of-scope path touched by nls-po-programs-server-tools-wave1-runtime lane: ${changed_file}"
  ...
  log "Nls/po/programs/server/tools wave1 runtime lane passed: ${patch_count} custom patch(es) on top of include-libs-wave1-runtime + dlls-wave1-runtime + kernel32-runtime + win32u-runtime + kernelbase-support + kernelbase-runtime + winebuild-support + libs-wine-support + wow64-struct-support + wow64-support + server-runtime + signal-runtime + loader-runtime + core-runtime + GN baseline"
}
