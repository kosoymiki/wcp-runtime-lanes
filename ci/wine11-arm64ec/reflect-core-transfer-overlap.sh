...
: "${WCP_WINE11_CORE_OVERLAP_OUT:=${ROOT_DIR}/docs/WINE11_ARM64EC_CORE_TRANSFER_REPORT.md}"
: "${WCP_WINE11_CORE_OVERLAP_WINDOW:=400}"
: "${WCP_WINE11_CORE_OVERLAP_LIMIT:=80}"

log() { printf '[wine11-core-overlap] %s\n' "$*" >&2; }
fail() { printf '[wine11-core-overlap][error] %s\n' "$*" >&2; exit 1; }
tmp_dir=""
...
main() {
  ...
  tmp_dir="$(mktemp -d /tmp/wine11-core-overlap-XXXXXX)"
  ...
  git -C "${WCP_LOCAL_VALVE_WINE_EXP10_DIR}" log --format= --name-only --max-count "${WCP_WINE11_CORE_OVERLAP_WINDOW}" "${valve_ref}" -- "${pathspecs[@]}" \
    | sed '/^$/d' \
    | sort \
    | uniq -c \
    | sort -nr \
    | head -n "${WCP_WINE11_CORE_OVERLAP_LIMIT}" > "${hot_list}"
  ...
  mkdir -p "$(dirname "${WCP_WINE11_CORE_OVERLAP_OUT}")"
  {
    printf '# Wine11 Core Transfer Overlap Report\n\n'
    printf 'This report compares the first safe Wine11 transfer lane against our current in-repo GameNative wine patch base.\n\n'
    printf '## Valve Source Window\n\n'
    printf '%s\n\n' "- Valve \`experimental_10.0\` head: \`${valve_head}\`"
    printf '%s\n\n' "- analyzed commit window: latest \`${WCP_WINE11_CORE_OVERLAP_WINDOW}\` commits"

    printf '## Core Lane Scope\n\n'
    printf '```text\n'
    cat "${PREFIX_FILE}"
    printf '```\n\n'
    ...
    printf '## Current Custom Lane Files\n\n'
    printf '```text\n'
    if [[ -s "${lane_list}" ]]; then
      cat "${lane_list}"
    else
      printf '(no custom core-runtime patches yet)\n'
    fi
    printf '```\n\n'

    printf '## Interpretation\n\n'
    printf 'Paths already listed under "Existing GN Wine Patch Overlap" are the safest first comparison points, because our repo already owns behavior there.\n'
    printf 'Paths hot in Valve but absent from GN are the next candidates, but they should enter only as narrow slices inside the core-runtime lane.\n'
  } > "${WCP_WINE11_CORE_OVERLAP_OUT}"

  log "Report written: ${WCP_WINE11_CORE_OVERLAP_OUT}"
}
