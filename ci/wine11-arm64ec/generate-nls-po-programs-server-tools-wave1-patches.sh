...
log() { printf '[wine11-npsst-wave1-gen] %s\n' "$*"; }
fail() { printf '[wine11-npsst-wave1-gen][error] %s\n' "$*" >&2; exit 1; }
tmp_dir=""
...
main() {
  ...
  tmp_dir="$(mktemp -d /tmp/wine11-npsst-wave1-gen-XXXXXX)"
  ...
  log "Base snapshot: AndreRH + GN + core/loader/signal/server/server-support/wow64/wow64-struct/libs-wine/winebuild/kernelbase/win32u/kernel32/dlls-wave1/include-libs"
